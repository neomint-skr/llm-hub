#!/bin/bash
# Test Build Script for Unified Gateway
...
echo "Unified Gateway Build Test"
echo "========================="
...
if netstat -an 2>/dev/null | grep -q ":8080.*LISTEN" || ss -ln 2>/dev/null | grep -q ":8080"; then
    echo "WARNING: Port 8080 is already in use, skipping port test"
    exit 0
fi
...
if curl -f http://localhost:8080/health 2>/dev/null; then
    ...
    echo "WARNING: Health endpoint not responding (expected without Bridge)"
fi
...
if echo "$LOGS" | grep -i "error" | grep -v "Bridge" | grep -v "connection"; then
